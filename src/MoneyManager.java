...
class MainFrame extends JFrame implements ActionListener {
    ...
    private final JMenu m_system = new JMenu("系统管理");
    private final JMenu m_fm = new JMenu("收支管理");
    private final JMenuItem[] mI = {new JMenuItem("密码重置"), new JMenuItem("退出系统")};
    private final JMenuItem m_FMEdit = new JMenuItem("收支编辑");
        ...
        if (b_cancel == e.getSource()) {
            ...
            if (t_oldPwd.length() == 0 || t_newPwd.length() == 0 || t_newPwdAgain.length() == 0) {          //判断密码是否为空
                ...
                if (t_newPwd.length() > 15 || t_newPwdAgain.length() > 15) {                               //判断密码长度
                    ...
                    try {
                        if (new SQLiteFunction().login(username, t_oldPwd)) {                                 //判断原密码是否正确
                            if (t_newPwd.equals(t_newPwdAgain)) {                                          //判断两次密码是否相同
                                new SQLiteFunction().changePwd(this.username, t_newPwd);                      //修改密码
                                this.dispose();
                            } else {
    ...
    public BalEditFrame() {
        ...
        table.addMouseListener(new MouseListener() {//为table添加鼠标点击事件监听addMouseListener
            ...
            public void mouseClicked(MouseEvent e) {
                        ...
                        selRowIndex = table.getSelectedRow();                               //取得用户所选行号
                        t_id.setText((String) table.getValueAt(selRowIndex, 0));     //把所选内容填入信息框
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (b_select == e.getSource()) {            //查询所有收支信息
                ...
                String selid = (String) table.getValueAt(selRowIndex, 0);       //选中某列
                if(new DataConfirm().isNotEmpty5(id, rdata, rtype, ritem, t_bal.getText()))
                    ...
                    JOptionPane.showMessageDialog(null,                                    //弹出格式错误警告
                            "各项均不能为空", "警告", JOptionPane.ERROR_MESSAGE);
                try{
                    format.parse(rdata);
                } catch (ParseException p){
                    JOptionPane.showMessageDialog(null,                                    //弹出格式错误警告
                            "日期格式应为：YYYYMMDD", "警告", JOptionPane.ERROR_MESSAGE);
                    new SQLiteFunction().deleteData(id);                                                       //删除错误数据
                }
                ...
                if (!new DataConfirm().isNotEmpty5(id, rdata, rtype, ritem, t_bal.getText()))
                    JOptionPane.showMessageDialog(null,                                    //弹出格式错误警告
                            "各项均不能为空", "警告", JOptionPane.ERROR_MESSAGE);
                if (new SQLiteFunction().judgeRepeat(id)) {  //ID不能重复！
                    JOptionPane.showMessageDialog(null,
